...
public enum UploadCreds {
    ...
    public static let lastPassName     = "Step Test Files API (dev and stage)"
    /// **Host** address of the server (`String`)
    public static let hostString       = "https://steptestfilesdev.uchicago.edu"
    
    /// The fill URL path to the uploads directory.
    ///
    /// Stop being fancy with building up from the host name.
    public static let fullUploadString = "https://steptestfilesdev.uchicago.edu/api/upload"
    
    /// UTTP  Basic iuser name (`String`)
    public static let userID           = "iosuser"
    /// UTTP  Basic password (`String`)
    public static let password         = "sliD3gtorydra"
#elseif BETA_API
    // Use for TestFlight
    /// The LastPass name for the credentials
    public static let lastPassName     = "Step Test Files API (dev and stage)"
    /// **Host** address of the server (`String`)
    public static let hostString       = "https://steptestfilesstage.uchicago.edu"
    
    /// The fill URL path to the uploads directory.
    ///
    /// Stop being fancy with building up from the host name.
    public static let fullUploadString = "https://steptestfilesstage.uchicago.edu/api/upload"
    ...
    /// The LastPass name for the credentials
    public static let lastPassName     = "Step Test Files API (PROD)"
    /// **Host** address of the server (`String`)
    public static let hostString       = "https://steptestfiles.uchicago.edu"
    /// The fill URL path to the uploads directory.
    ///
    /// Stop being fancy with building up from the host name.
    public static let fullUploadString = "https://steptestfiles.uchicago.edu/api/upload"
    /// UTTP  Basic iuser name (`String`)
    public static let userID           = "iosuser"
    /// UTTP  Basic password (`String`)
    public static let password         = "wayB3aundanar"
#endif
    
    /// `URL` for the host server (derived from `hostString`)
    public static let hostURL           = URL(string: hostString)!
    /// Components of the upload API path (`[String]`) to append to the `hostURL`. Used because `appendingPathComponent` was erroneous
    public static let pathComps         = ["api", "upload"]
    /// API path on the host for uploading. (`String`).
    public static let hostPathComponent = "/api/upload"
    /// `URL` for the fuily-qualified API address.
    public static let uploadDirURL      = pathComps.reduce(hostURL) { partURL, component in
