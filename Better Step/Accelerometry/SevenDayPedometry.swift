...
final class Pedometry {
    ...
    typealias PedometerResult = Result<CMPedometerData, Error>
    typealias PedometerCallback = (PedometerResult) -> Void
    func dateSpan(completion: @escaping PedometerCallback) {
        ...
        pedometer.queryPedometerData(from: sevenDaysBefore,
                                     to: lastMidnight)
        { pedData, error in
            // It's escaping. You can't really signal failure.
            let retval: PedometerResult
            if let error {
